...
import {
  Archive, BarChart3, ClipboardList, Crown, FileText, Settings, Users,
} from 'lucide-react'
...
const ACCENT = '#f59e0b' // Amber — Facturation
...
export default function FacturationLayout() {
  ...
  const [userInfo, setUserInfo] = useState({
    firstName: 'Gestionnaire', lastName: 'Facturation', email: '', department: 'Comptabilité', role: ''
  })
  ...
  if (blocked) return <ModuleDisabledView accentColor={ACCENT} moduleLabel="Facturation" />

  const isAdmin = userInfo.role === 'admin_principal'

  const navItems = [
    { to: '/facturation/dashboard', label: 'Dashboard',  Icon: BarChart3      },
    { to: '/facturation/orders',    label: 'Commandes',  Icon: ClipboardList  },
    { to: '/facturation/clients',   label: 'Clients',    Icon: Users          },
    { to: '/facturation/invoices',  label: 'Factures',   Icon: FileText       },
    { to: '/facturation/archive',   label: 'Archive',    Icon: Archive        },
    { to: '/facturation/settings',  label: 'Paramètres', Icon: Settings       },
  ]
      ...
      <main className={cn('main-content', collapsed && 'collapsed')}>
        <header className="page-header-bar">
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-[var(--fg)] truncate">Gestion Facturation</h1>
            <p className="text-xs text-[var(--fg-muted)] truncate hidden sm:block">
              Commandes, clients, factures et archives
            </p>
          </div>
